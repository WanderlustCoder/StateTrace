...
| ST-G-007 | Telemetry bundle integration | Backlog | Release/Telemetry | Update `docs/Release.md` checklist + scheduled verification harness to require a `Tools/New-TelemetryBundle.ps1` artifact link, run `Tools\Test-TelemetryBundleReadiness.ps1 -IncludeReadmeHash -SummaryPath` per `docs/runbooks/Telemetry_Bundle_Verification.md`, and log the bundle README hashes + summary path before approvals. | docs/plans/PlanG_ReleaseGovernance.md |
