...
| ST-B-009 | Toggle skip-site-cache policy safely | In Progress | Ingestion | Guard module now flips `SkipSiteCacheUpdate` off/on for pipeline (`-DisableSkipSiteCacheUpdate`), warm-run telemetry, and shared-cache warmup; runbook updated. Warm harness now emits comparisons even when DB writes are sparse. Latest guarded run (`WarmRunTelemetry-20251201-guard-skipdiv7.json`, port diversity guard skipped) improved vs cold (+8.95%: warm avg 898.585 ms, cold 986.951 ms) with warm providers Cache=2,302 / Refresh=182 / Hydrate=20 / MissingDatabase=62 (`WarmCacheHitRatioPercent=89.71`, SignatureMatchMissCount=176); queue summary emitted; shared-cache analyzer shows `SnapshotImported=1` (still low) and `GetMiss=170`. Earlier guarded run (logs moved to `DebugLogs/`) had warm AccessCacheHit=14/AccessRefresh=0 with queue delay p95/p99=0.47/0.87 ms. Snapshot export path hardened (cache snapshot enumeration falls back to preserved snapshot store and ParserWorker retries empty exports with rehydration; Pester coverage added 2025-12-10); mock-corpus pipeline still emits empty snapshot despite diagnostics (`SnapshotImported=24`, `GetHit=118`, `GetMiss=560`)—need full corpus warm run to verify SITE1 host maps populate. Queue delay summary now carries SampleCount in the stats and history updater falls back to Count/root values to avoid missing-property warnings. | docs/plans/PlanB_Performance.md |
