...
| ST-E-003 | Schedule daily rollup harness | Ready for Review | Telemetry | `schtasks /Query /TN StateTraceDailyRollup /V /FO LIST` (2025-11-13 13:35 MT) shows `Last Result: 0` for the wrapper command `powershell.exe -NoProfile -ExecutionPolicy Bypass -File Tools\Invoke-DailyRollupScheduled.ps1 -Days 1 -IncludePerSite -IncludeSiteCache`; run produced `Logs/IngestionMetrics/IngestionMetricsSummary-20251113-133544.csv` and updated telemetry bundle notes. | docs/plans/PlanE_Telemetry.md, docs/runbooks/Schedule_Daily_Rollup.md |
