...
## Current board snapshot (2025-11-12)
| ID | Title | Column | Owner/Role | Deliverable | Links |
|----|-------|--------|------------|-------------|-------|
| ST-B-001 | Investigate WLLS snapshot/materialize regression | In Progress (WIP 1/2) | Ingestion/Performance | DeviceRepository now keeps typed host maps intact when exporting shared cache snapshots (new `Convert-InterfaceSiteCacheHostMapToExportMap` logic + tests). Cold runs still show 50 WLLS hosts hydrating (avg 336 ms, max 471 ms), but the latest preserved warm regression (`Tools/Invoke-WarmRunRegression.ps1 -VerboseParsing`, 2025-11-12 ~17:35 MT) only logged 124/218 cache hits (WLLS 50 misses, BOYO 24) and `SiteCacheProviderReason=AccessRefresh` on 74 hosts, so `AssertWarmCache` continues to fail with no new `WarmRunTelemetry-*.json`. Need to root-cause the AccessRefresh reasons before another telemetry export. <br> - 2025-11-12 19:35 MT: Ran `Tools\Analyze-SharedCacheStoreState.ps1` + `Tools\Analyze-SiteCacheProviderReasons.ps1` (see `docs/CODEX_SHARED_CACHE_DIAGNOSTICS.md`). `SnapshotImported=0`, `InitNewStore=25`, `GetMiss=1,897` vs. `GetHit=1,842`; WLLS alone generated 1,148 misses and 192 `AccessRefresh` events (top hosts still 400-660 ms fetch). <br> - 2025-11-12 19:55 MT: Executed `Tools\Invoke-StateTracePipeline.ps1 -SkipTests -VerboseParsing -SharedCacheSnapshotPath Logs/SharedCacheSnapshot-20251110-192915.clixml -RunSharedCacheDiagnostics`. Snapshot restore reported 7 entries but diagnostics still show `SnapshotImported=0`, `InitNewStore=28`, `GetMiss=1,924` vs. `GetHit=2,066` and WLLS 216 `AccessRefresh` events. Conclusion: shared store is still reset after the cold pass even when we seed the snapshot; next step is to trace why [StateTrace.Repository.SharedSiteInterfaceCacheHolder] clears between parser worker loads. | docs/plans/PlanB_Performance.md |
