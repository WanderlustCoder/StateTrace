...
| ST-D-010 | Investigate WLLS->BOYO idle gap | In Progress | UI + Scheduler | Site-pair, timeline, site-mix, sweep-order, and diversity guard outputs (`docs/performance/PortBatchSiteGapSummary-20251113.md`, `docs/performance/WLLS_BOYO_GapTimeline-20251113.md`, `docs/performance/PortBatchSiteMix-20251113.md`, `docs/performance/DispatchHarnessSweepOrder-20251113.md`, `Logs/Reports/PortBatchSiteDiversity-20251114.json`) now pair with the scheduler comparison (`docs/performance/SchedulerVsPortDiversity-2025-11-14.md` + `Logs/Reports/SchedulerVsPortDiversity-2025-11-14.json`) and the balanced Hostname dropdown ordering (`DeviceCatalogModule\Get-DeviceSummaries` now rotates sites), proving the parser guard caps streaks at 1 while the UI replay still runs WLLS 25 times. Next step: inspect dispatcher harness timelines / autoscale outputs to explain the replay and validate the balanced host list (`Data/RoutingHosts_Balanced.txt` via `Tools/New-BalancedRoutingHostList.ps1`). | docs/plans/PlanD_FeatureExpansion.md |
