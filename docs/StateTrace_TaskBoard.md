...
| ST-E-003 | Schedule daily rollup harness | Ready | Telemetry | Use `Tools/Schedule-DailyRollupTask.ps1` (or CI) to register the daily rollup harness, then log the cadence + `Logs/IngestionMetrics/IngestionMetricsSummary-<timestamp>.csv` path in Plan E. | docs/plans/PlanE_Telemetry.md |
