...
| ST-E-007 | Release-readiness telemetry bundle | Backlog | Telemetry/PMO | Produce `Logs/TelemetryBundles/<date>/` bundles via `Tools/Publish-TelemetryBundle.ps1` (auto-discovers rollup CSVs, analyzers, warm-run summaries, doc-sync references) and verify them per `docs/runbooks/Telemetry_Bundle_Verification.md` (README hash + readiness output logged). | docs/plans/PlanE_Telemetry.md |
...
| ST-G-007 | Telemetry bundle integration | In Progress | Release/Telemetry | Update `docs/Release.md` checklist + scheduled verification harness to require a `Tools/New-TelemetryBundle.ps1` artifact link, run `Tools\Test-TelemetryBundleReadiness.ps1 -IncludeReadmeHash -SummaryPath` per `docs/runbooks/Telemetry_Bundle_Verification.md`, and log the bundle README hashes + summary path before approvals.  2025-12-23: Invoke-StateTraceScheduledVerification.ps1 now accepts -TelemetryBundlePath/-VerifyTelemetryBundleReadiness; schedule runbook notes -AdditionalArguments for bundle checks. | docs/plans/PlanG_ReleaseGovernance.md |
