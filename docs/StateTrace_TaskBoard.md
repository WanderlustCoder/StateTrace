...
| ST-K-005 | Code review findings remediation (batch 1) | In Progress | Automation | Address high-severity items from `docs/notes/2025-12-21_code_review_findings.md` and keep status updates logged there; findings 4-6, 7-14, 15-33, 34-54, 55-63, 65-69, 70-76, 72, 88-98, and 99-100 complete (schema warnings, DeviceDetails/Insights diagnostics, parser/filter/repo cleanup warnings, cache event caps, module import warnings, tooling JSON helper, streamed telemetry parsing, cache sync, bounded polling/timeouts, export dialog suppression with view/headless opt-in). | docs/plans/PlanK_DeveloperExperience.md |
