...
| ST-A-005 | Dispatcher alert integration | In Progress | Ingestion/Telemetry | Dispatcher harness flushes telemetry buffer; queue delay summary `Logs/IngestionMetrics/QueueDelaySummary-2025-12-22.json` (20 samples, QueueBuildDelay p95 25.60 ms, p99 33.57 ms) sourced from `Logs/IngestionMetrics/2025-12-22.json`. Scheduler telemetry now flushes at parse completion and analyzer ignores synthesized events when real launches exist; pipeline run passes scheduler fairness (max streak 1) in `Logs/Reports/ParserSchedulerLaunch-2025-12-22.json` and `docs/performance/SchedulerVsPortDiversity-2025-12-22.md`. Port batch diversity guard uses synthesized events; `Logs/Reports/PortBatchSiteDiversity-2025-12-22.json` max streak 1. | docs/plans/PlanA_RoutingReliability.md |
| ST-A-006 | Routing evidence bundle linkage | In Progress | Ingestion/PMO | Routing bundle published at `Logs/TelemetryBundles/Routing-20251113/Routing/` (queue summary + dispatcher harness logs + session doc); repeat after future runs. | docs/plans/PlanA_RoutingReliability.md |
