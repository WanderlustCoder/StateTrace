...
- **Automate warm-run regression in verification pipeline** - [Automation][Telemetry] Deliverable: run `Tools/Invoke-StateTraceVerification.ps1` in scheduled/CI jobs with `-RunWarmRunRegression`, archive `WarmRunTelemetry-*.json`, and enforce `AssertWarmCache` thresholds so cache regressions are caught immediately.
  ...
  - 2025-10-24 11:22 MT: Latest preserved-session run (`Tools/Invoke-WarmRunRegression.ps1 -VerboseParsing`) exported `Logs/IngestionMetrics/WarmRunTelemetry-20251024-105636.json` with cold/warm averages 415.9 ms to 323.8 ms (22.1% gain) and `WarmCacheProviderHitCount=37`. Follow-up: reconcile the lower improvement versus the prior 43.6% run and fold the new thresholds into the verification alerting plan.
