...
| ST-L-004 | Move UI services out of MainWindow | UI | Done - 2026-01-04 | Created `Modules/MainWindow.Services.psm1` with 10 exported functions: settings management (Get/Set-StateTraceSettings), user-action telemetry (Publish-UserActionTelemetry), freshness queries (Get-SiteIngestionInfo, Get-SiteCacheProviderFromMetrics, Get-FreshnessStatus), and parser job helpers (Get-ParserLogTail, Get-ParserJobStatus, Get-LatestPipelineLogPath). Added 25 Pester tests in `Modules/Tests/MainWindow.Services.Tests.ps1` with `-Tag Decomposition,Services`. All tests pass. |
...
## Recently delivered
- ST-L-004: Created `MainWindow.Services.psm1` (530 lines) extracting UI service logic: settings, freshness, telemetry, parser job status. 25 Pester tests added, all 56 `-Tag Decomposition` tests now pass.
