...
| ST-G-007 | Telemetry bundle integration | Release + Telemetry | Backlog | Consume Plan E ST-E-007 bundles inside the release checklist; verify each bundle contains rollup CSVs, shared-cache analyzers, warm-run summaries, and risk deltas before sign-off (follow `docs/runbooks/Telemetry_Bundle_Verification.md`). |
