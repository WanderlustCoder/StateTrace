...
| ST-F-002 | Access scrubber automation | Data | Done - 2026-01-04 | Created `Tools/Invoke-SanitizationWorkflow.ps1` wrapper with standardized paths (`Data/Postmortems/<IncidentId>/Sanitized`, `Logs/Sanitization/<IncidentId>.json`), history tracking (`Logs/Sanitization/SanitizationHistory.csv`), `-RefreshFixtures` switch to copy to `Tests/Fixtures/Postmortems/`, and `-DryRun` mode. |
...
| ST-F-004 | Identity/RBAC rollout playbook | Security / Platform | Done - 2026-01-04 | Created `docs/runbooks/Identity_RBAC_Rollout.md` with dev seat bootstrap steps, RBAC switch verification commands, security considerations, and telemetry integration. Covers AD-integrated accounts (Phase 1) and local operator fallback for air-gapped sites. |
