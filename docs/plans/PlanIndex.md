...
- **Plan V** - **In Progress (5/6 Done)**. Lightweight IPAM with VLAN registry, subnet planning, and conflict detection. Core module, tests, and UI view integrated into MainWindow. VLAN discovery implemented: Import-VLANsFromConfig, Import-SVIsFromConfig, Import-VLANsToDatabase, Merge-VLANDiscovery, New-VLANDiscoveryReport. 61 tests passing. Full wizard UI pending.
- **Plan W** - **In Progress (4/6 Done)**. Network topology visualization with link discovery from interface descriptions, 4 layout algorithms (hierarchical, force-directed, circular, grid), impact analysis, and export (SVG, JSON, Draw.io). Core module, tests, and UI view integrated into MainWindow.
