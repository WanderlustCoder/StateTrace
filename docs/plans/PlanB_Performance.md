...
| ST-B-006 | Automate warm-run diff hotspot triage | Automation | Backlog | Integrate `Tools\Analyze-WarmRunDiffHotspots.ps1 -TelemetryPath Logs\IngestionMetrics\WarmRunTelemetry-<run>.json -Top 25` into the warm regression script, export CSV output, and link the top offenders in the plan/task updates. |
| ST-B-007 | Enforce shared-cache analyzers in verification | Automation | Backlog | Extend `Tools\Invoke-StateTraceVerification.ps1` / `Tools\Invoke-StateTraceScheduledVerification.ps1` to run the shared-cache analyzers and fail verification when `SnapshotImported=0` or `AccessRefresh` dominates. Document the workflow in `docs/CODEX_SHARED_CACHE_DIAGNOSTICS.md`. |
...
- Autoscale / overrides: `Import-Module .\Modules\ParserRunspaceModule.psm1; Get-AutoScaleConcurrencyProfile -DeviceFiles <paths>` plus `-ThreadCeilingOverride`, `-MaxWorkersPerSiteOverride`, etc., all recorded in the plan/task board and reset after each run.
