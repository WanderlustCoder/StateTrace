...
| ST-H-002 | Expose data freshness & pipeline health in the UI | UI / Telemetry | Done - 2026-01-04 | Implemented enhanced status strip with: (1) Colored freshness indicator (Green <24h, Yellow 24-48h, Orange 2-7d, Red >7d, Gray unknown); (2) Pipeline health label showing last run time; (3) View Log button to open latest pipeline log; (4) Reload DB button to refresh from Access without parsing. Pester tests added (`Modules/Tests/StatusStripIndicators.Tests.ps1`, 19 tests passing). Runbook updated with indicator semantics (`docs/StateTrace_Operators_Runbook.md`). |
| ST-H-003 | Instrument user-facing telemetry + feedback loop | Telemetry / UI | Done - 2026-01-04 | `UserAction` events emit for Scan Logs, Load from DB, HelpQuickstart, InterfacesView, CompareView, SpanSnapshot, and RefreshFromDb (optional). Adoption thresholds codified in `docs/telemetry/Automation_Gates.md`: 100% required action coverage (6 actions), ≥10 events per bundle, ≥2 sites. Rollups track `UserActionCoverage` metric with missing action detection. Latest evidence: `Logs/Reports/UserActionSummary-20251126-run3.json` bundled in `Logs/TelemetryBundles/UI-20251126-useraction7/`. |
