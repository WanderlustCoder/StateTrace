...
|----|-------|-------|--------|-------|
| ST-E-003 | Schedule the daily rollup harness | Telemetry | Ready for Review | `pwsh Tools\Schedule-DailyRollupTask.ps1 -TaskName StateTraceDailyRollup -StartTime 02:00 -Force` now registers a task that launches `Tools\Invoke-DailyRollupScheduled.ps1 -Days 1 -IncludePerSite -IncludeSiteCache`. `schtasks /Query /TN StateTraceDailyRollup /V /FO LIST` (2025-11-13 13:35 MT) reports `Last Result: 0`, and the run produced `Logs/IngestionMetrics/IngestionMetricsSummary-20251113-133544.csv` plus bundle-ready analyzer output. |
