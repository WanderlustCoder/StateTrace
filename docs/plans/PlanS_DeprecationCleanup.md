...
| ST-S-003 | Script/runbook pruning | Automation | Done - 2026-01-04 | Created `Tools\Report-UnusedScripts.ps1` to identify potentially unused scripts by scanning for references across Tools/Modules/docs. Includes allowlist of known entry points (27 scripts). Initial scan found 2 unreferenced scripts for review. Supports `-FailOnUnused` for CI and `-Allowlist` for custom entry points. |
| ST-S-004 | UI cleanup sweep | UI | Done - 2026-01-04 | Created `Tools\Invoke-UiCleanupAudit.ps1` to identify unused XAML controls (x:Name without code refs), orphaned event handlers, commented controls, and legacy naming patterns. Scans 9 views, found 3 cleanup candidates and 6 legacy patterns requiring manual review. Output to `Logs/Reports/UiCleanupAudit-*.json`. |
