...
| 2. Pick the work | Find a task and its context | `docs/taskboard/TaskBoard.csv`, `docs/CODEX_BACKLOG.md`, `docs/plans/PlanIndex.md` + `Plan*.md`, `docs/CODEX_PLAN_AUTOMATION_MATRIX.md` | Task board = current state, backlog = automation view, plan files = objectives/timelines, matrix = commands/telemetry per plan. |
...
| 5. Validate | Tests + telemetry capture | `docs/CODEX_RUNBOOK.md` (validation rows), plan file telemetry sections | Record outputs in session log and plan timeline; run `Tools\Test-TelemetryBundleReadiness.ps1` (or `Tools\Invoke-AllChecks.ps1 -TelemetryBundlePath …`) whenever telemetry bundles are touched. |
| 6. Document progress | Update plan, task board, backlog | `docs/plans/Plan*.md`, `docs/StateTrace_TaskBoard.md`, `docs/taskboard/TaskBoard.csv`, `docs/CODEX_BACKLOG.md`, `docs/CODEX_DOC_SYNC_PLAYBOOK.md` | Use the playbook to keep every doc in sync for autonomous pickup. |
