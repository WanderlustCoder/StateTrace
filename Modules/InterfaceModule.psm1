...
function Get-InterfaceInfo {
    ...
    # Use a safe lookup to avoid errors when the variable is not defined under StrictMode.
    $debug = $false
    try {
        $varDebug = Get-Variable -Name 'StateTraceDebug' -Scope Global -ErrorAction SilentlyContinue
        if ($varDebug) {
            $debug = ($varDebug.Value -eq $true)
        }
    } catch {
        $debug = $false
    }
    try {
        ...
        if ($debug) {
            ...
            # $authBlockLines may not be an array if the DB returned a single string.
            # Wrap in @() so .Count works on a single item as well.
        ...
        foreach ($row in ($dt | Select-Object Port, Name, Status, VLAN, Duplex, Speed, Type, LearnedMACs, AuthState, AuthMode, AuthClientMAC, AuthTemplate, Config, PortColor, ConfigStatus, ToolTip)) {
            # The AuthTemplate value recorded in the database may not reflect the actual
            # configuration of the port.  We attempt to compute the best matching
            # template by inspecting the interface's configuration and comparing
            # it against the templates' required and excluded commands.  This
            # prevents ports missing key lines (e.g. dot1x port-control auto) from
            # being incorrectly labelled as dual-auth or flexible.
            $dbAuthTemplate = $row.AuthTemplate
            # Lookup template by DB value (name or alias)
            $match = $null
            if ($templates) {
                $match = $templates | Where-Object {
                    $_.name -ieq $dbAuthTemplate -or
                    ($_.aliases -and ($_.aliases -contains $dbAuthTemplate))
                } | Select-Object -First 1
            }
            # Split configuration into trimmed non-empty lines for matching
            $configLines = @()
            if ($row.Config) {
                try {
                    $configLines = ('' + $row.Config) -split "`r?`n" | ForEach-Object { $_.Trim() } | Where-Object { $_ }
                } catch {
                    $configLines = @()
                }
            }
            # Attempt to compute the best matching template based on config
            $computedMatch = $null
            if ($templates -and $configLines.Count -gt 0) {
                foreach ($tmpl in $templates) {
                    # Gather required/excluded commands; treat missing as empty.  Use the
                    # array subexpression operator @() to ensure a single string is wrapped
                    # into an array.  Without this, a lone string is treated as a
                    # collection of characters and does not have a Count property.
                    $reqs = @()
                    if ($tmpl.PSObject.Properties['required_commands']) {
                        $reqs = @($tmpl.required_commands)
                    }
                    $excl = @()
                    if ($tmpl.PSObject.Properties['excluded_commands']) {
                        $excl = @($tmpl.excluded_commands)
                    }
                    $matchType = if ($tmpl.PSObject.Properties['match_type']) { '' + $tmpl.match_type } else { 'contains_all' }
                    # Lowercase config and template commands
                    $lowerConfig = $configLines | ForEach-Object { $_.ToLower() }
                    $lowerReqs   = $reqs | ForEach-Object { ('' + $_).ToLower() }
                    $lowerExcl   = $excl | ForEach-Object { ('' + $_).ToLower() }
                    $hasAll = $true
                    $hasAny = $false
                    # Use the array subexpression @(...) to force enumeration when a
                    # single string was provided.  Without @(), a single string
                    # remains a System.String object, which does not expose the Count
                    # property.  Wrapping in @() ensures .Count exists and counts
                    # the number of strings (commands) rather than characters.
                    $reqCount  = @($lowerReqs).Count
                    $exclCount = @($lowerExcl).Count
                    if ($reqCount -gt 0) {
                        foreach ($rc in $lowerReqs) {
                            $found = $false
                            foreach ($cfgLine in $lowerConfig) {
                                if ($cfgLine -like ("$rc*")) { $found = $true; break }
                            }
                            if (-not $found) { $hasAll = $false }
                            if ($found) { $hasAny = $true }
                        }
                    } else {
                        # No required commands specified means any config qualifies
                        $hasAll = $true
                        $hasAny = $true
                    }
                    # Check excluded commands: any excluded command disqualifies this template
                    $hasExcluded = $false
                    if ($exclCount -gt 0) {
                        foreach ($ex in $lowerExcl) {
                            foreach ($cfgLine in $lowerConfig) {
                                if ($cfgLine -like ("$ex*")) { $hasExcluded = $true; break }
                            }
                    ...
                    switch ($matchType.ToLower()) {
                        'contains_all' { $isMatch = ($hasAll -and (-not $hasExcluded)); break }
                        'contains_any' { $isMatch = ($hasAny -and (-not $hasExcluded)); break }
                        default        { $isMatch = ($hasAll -and (-not $hasExcluded)) }
                    }
                    if ($isMatch) { $computedMatch = $tmpl; break }
                }
            }
            if (-not $computedMatch) { $computedMatch = $match }
            # Determine colour and status.  Use the template associated with
            # the parser‑assigned AuthTemplate wherever possible.  The parser
            # classification takes global authentication ranges into account.
            # Only fall back to a computed match when no database template
            # association can be found.  If neither is available, leave the
            # colour gray and status mismatch.
            $portColor = 'Gray'
            $configStatus = 'Mismatch'
            $tmplToUse = $null
            if ($match) {
                # $match corresponds to the parser‑assigned AuthTemplate
                $tmplToUse = $match
            } elseif ($computedMatch) {
                # Use the computed match only when no parser template is available
                $tmplToUse = $computedMatch
            }
            if ($tmplToUse) {
                $portColor = if ($tmplToUse.color) { $tmplToUse.color } else { 'Gray' }
            ...
            $toolTipCore = if ($row.ToolTip) {
                ...
                if ($cfg -and $cfg.Trim() -ne '') {
                    "AuthTemplate: $dbAuthTemplate`r`n`r`n$cfg"
                } else {
                    "AuthTemplate: $dbAuthTemplate"
                }
            }
            $toolTip = $toolTipCore
            ...
            if ($debug) {
                ...
                if ($computedMatch) {
                    Write-Host ([string]::Format("[Get-InterfaceInfo] Computed template for {0}:{1} is {2}", $Hostname, $row.Port, $computedMatch.name)) -ForegroundColor DarkGreen
                } else {
                    Write-Host ([string]::Format("[Get-InterfaceInfo] No computed template match for {0}:{1}", $Hostname, $row.Port)) -ForegroundColor DarkYellow
                }
            }
            # Build result object including computed template name for debugging/reference
            $results += [PSCustomObject]@{
                ...
                PortColor     = $portColor
                ComputedTemplate = if ($computedMatch) { $computedMatch.name } else { $null }
            }
        }
        return $results
    } catch {
...
function New-InterfacesView {
    ...
    if ($compareButton) {
        ...
        try {
            ...
            # Expand the compare sidebar if collapsed.  With configuration
            # panes now at a 280 pixel minimum width each, a sidebar width of
            # around 600 pixels comfortably fits both without consuming too
            # much of the main window.  Adjust the column width only if
            # currently zero (hidden).
    ...
    if ($configureButton -and $interfacesGrid -and $templateDropdown) {
        $configureButton.Add_Click({
            ...
            $selectedRows = @($grid.SelectedItems)
            # Collect checked rows (those whose IsSelected property is $true)
            $itemsEnum = @()
            if ($grid.ItemsSource -is [System.Collections.IEnumerable]) {
                $itemsEnum = @($grid.ItemsSource)
            }
            ...
            # Prefer checked rows; fall back to selected rows
            ...
            try {
                ...
                foreach ($int in $selected) {
                    if ($int.PSObject.Properties['Name'] -and $int.Name -ne '') { $namesMap[$int.Port] = $int.Name }
                    if ($int.PSObject.Properties['VLAN'] -and $int.VLAN -ne '') { $vlansMap[$int.Port] = $int.VLAN }
                }
    ...
    if ($copyDetailsButton -and $interfacesGrid) {
        $copyDetailsButton.Add_Click({
            # Use global interfaces grid to read selected or checked items
            $grid = $global:interfacesGrid
            if (-not $grid) {
                [System.Windows.MessageBox]::Show("Interfaces grid not found.")
                return
            ...
            # Collect highlighted rows
            $selectedRows = @($grid.SelectedItems)
            # Collect rows with IsSelected checkboxes
            $itemsEnum = @()
            if ($grid.ItemsSource -is [System.Collections.IEnumerable]) {
                $itemsEnum = @($grid.ItemsSource)
            }
            ...
            # Use checked rows if any, otherwise highlighted rows
            if ($checkedRows.Count -gt 0) {
                $selected = $checkedRows
            } else {
                $selected = $selectedRows
            ...
            $output = foreach ($int in $selected) {
                # Determine which configuration text to include for this interface.  Prefer
                # the ToolTip string, which already includes the AuthTemplate header and any
                # appended global auth block, as this provides technicians with the
                # template name and complete configuration.  If ToolTip is not available,
                # fall back to composing a string from the AuthTemplate and Config properties.
                $intCfg = ''
                if ($int.ToolTip) {
                    $intCfg = $int.ToolTip
                } elseif ($int.PSObject.Properties['AuthTemplate'] -and $int.PSObject.Properties['Config']) {
                    $tmplName = '' + $int.AuthTemplate
                    $cfgVal   = '' + $int.Config
                    if ($cfgVal -and $cfgVal.Trim() -ne '') {
                        $intCfg = "AuthTemplate: $tmplName`r`n`r`n$cfgVal"
                    } else {
                        $intCfg = "AuthTemplate: $tmplName"
                    }
                }
                @(
                    "Port:        $($int.Port)",
