...
$siteCacheTemplateDurationMs = 0.0
    ...
    try {
        ...
        $bulkMetricsPropertyNames = if ($bulkMetrics) { @($bulkMetrics.PSObject.Properties.Name) } else { @() }
    ...
    $interfaceSyncTelemetryPayload = @{
        Hostname = $Hostname
        Site = $siteCodeValue
        UiCloneDurationMs = if ($bulkMetricsPropertyNames -contains 'UiCloneDurationMs') { [double]$bulkMetrics.UiCloneDurationMs } else { 0.0 }
        ...
        BulkRowsCommitted = if ($bulkMetricsPropertyNames -contains 'RowsStaged') { [int]$bulkMetrics.RowsStaged } else { 0 }
        BulkBatchId = if ($bulkMetricsPropertyNames -contains 'BatchId') { [string]$bulkMetrics.BatchId } else { $null }
        StreamDispatchDurationMs = if ($bulkMetricsPropertyNames -contains 'StreamDispatchDurationMs') { [double]$bulkMetrics.StreamDispatchDurationMs } else { 0.0 }
        StreamCloneDurationMs = if ($bulkMetricsPropertyNames -contains 'StreamCloneDurationMs') { [double]$bulkMetrics.StreamCloneDurationMs } else { 0.0 }
        StreamStateUpdateDurationMs = if ($bulkMetricsPropertyNames -contains 'StreamStateUpdateDurationMs') { [double]$bulkMetrics.StreamStateUpdateDurationMs } else { 0.0 }
        StreamRowsReceived = if ($bulkMetricsPropertyNames -contains 'StreamRowsReceived') { [int]$bulkMetrics.StreamRowsReceived } else { 0 }
        StreamRowsReused = if ($bulkMetricsPropertyNames -contains 'StreamRowsReused') { [int]$bulkMetrics.StreamRowsReused } else { 0 }
        StreamRowsCloned = if ($bulkMetricsPropertyNames -contains 'StreamRowsCloned') { [int]$bulkMetrics.StreamRowsCloned } else { 0 }
